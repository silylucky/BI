...
```text
Read .cursor/automate/sop/self-evolution-github.md
Read .cursor/automate/skills/evolution-orchestrator-github/SKILL.md
```
...
- G0/P5 使用 `*-github` subagent 与 GitHub MCP；G1-P4 使用共用 `evolution-*` subagent。
- **Shell git** 负责 checkout、branch、commit、fetch、rebase、push。
- **GitHub MCP** 负责 PR 查询、创建、Draft 转正、checks/mergeable 查询、Squash merge、评论、删分支。
- **禁止** GitLab MCP；**禁止** Shell 直接 merge PR；**禁止** GitHub MCP 做本地 rebase。
- PR base 必须等于 P3 记录的 `base_branch`，禁止默认 `main` / `dev` / `master`。

细节以 `skills/evolution-pr-gate-github/SKILL.md` 与 `skills/evolution-pr-finish-github/SKILL.md` 为准。
...
```text
G0 evolution-pr-gatekeeper-github
  ...
  → P5 evolution-pr-finisher-github
```

固定纪律：

- 有 Open PR 时只处理 PR，禁止进入 G1-P4。
...
| 门控 | 通过条件 | 未通过 |
|------|----------|--------|
| G0 | 无 Open PR 且工作区干净；或遗留 PR 已 Squash merge；dirty 但 state 可恢复则 `gate: RESUME` | 停止，只处理 PR；state 不可恢复则 BLOCKED |
...
| P4 | 完整验证 exit_code = 0 | 禁止建 PR |
| P5 | PRD/state 对齐及已有 plan 勾选已随同一 PR 提交，PR 已 Squash merge | 本轮结束 |
...
|------|----------|-------|----------|
| G0 | `evolution-pr-gatekeeper-github` | `evolution-pr-gate-github` | `gate: PASS/RESUME/BLOCKED` |
...
| P5 | `evolution-pr-finisher-github` | `evolution-pr-finish-github` + `prd-scoring` | PR 合并、PRD/state 对齐 |

`platform-pr-finisher` 在 GitHub 流程中固定映射到 `evolution-pr-finisher-github`。
...
- GitHub G0/P5 PR 规程：`skills/evolution-pr-gate-github/SKILL.md`、`skills/evolution-pr-finish-github/SKILL.md`
...
- 禁止未验证就建 PR；禁止跳过 rebase/冲突解决；禁止 Draft PR 遗留。
- 禁止 PR 合并到与 `base_branch` 不一致的目标分支。
- 禁止合并后不更新 PRD 8 维评分与 `evolution-state.md`。
- 禁止 G0/P5 委派无 `-github` 后缀的 PR subagent。
