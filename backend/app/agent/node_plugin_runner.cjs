...
function normalizeParameters(parameters) {
  ...
  return parameters.type ? parameters : { type: 'object', properties: parameters }
}
...
function discover(payload) {
  ...
  if (typeof mod.run === 'function') {
    return {
      ...
      protocol: 'exec_tool',
      tool: {
        name: path.basename(payload.modulePath, path.extname(payload.modulePath)),
        description: '外部插件执行工具',
...
async function invoke(payload) {
  ...
  if (payload.protocol === 'deeptalk_tool') {
    result = await mod.tool.run(
      { workspaceRoot: context.workspaceRoot || payload.pluginRoot },
      { id: `agent-${Date.now()}`, input: payload.args || {} },
    )
    ...
    result = await mod.run(payload.args || {}, {
